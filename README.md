...
- **Market price (automatic):** once the name is detected, the app searches UK shopping listings and fills the **Market price** field with the **average of the three cheapest** in-stock prices. This uses Claude's web search, which you must switch on once: in the Anthropic Console go to **Settings → Privacy** and enable web search. (Web search costs about 1p per lookup on top of normal token cost.)
...
- **Vivino:** Vivino has no public API, so score and price are fields you fill in (the Vivino button makes the lookup quick). Auto-fetching could be explored later but isn't reliable.
